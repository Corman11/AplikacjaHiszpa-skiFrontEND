...
// DOdać opis zadania
 ...
 responsable se presenta como una alternativa para reducir 
 el impacto ambiental y social.
    ...
    ${taskFinished && (checkIfLabelCorrect(0) ? styles.labelOutlineSet : styles.labelOutlineWrong)}`}
                ...
                <p/>
         Por un lado, el consumo responsable contribuye 
a la sostenibilidad.  <p/>
                  <span
                  className={`${styles.labelOutline}
    ...
    ${taskFinished && (checkIfLabelCorrect(1) ? styles.labelOutlineSet : styles.labelOutlineWrong)}`}
       ...
       Además, esta práctica fomenta 
 la solidaridad, ya que muchas marcas apoyan proyectos 
 sociales.<p/>
        <span
                  className={`${styles.labelOutline}
...
Imaginemos que una familia quisiera 
comprar solo productos locales: si hubiera más opciones 
asequibles, sería posible, pero la realidad es que los 
precios son altos y la oferta limitada. 
Además, la publicidad incentiva el consumo excesivo,
 lo que dificulta mantener hábitos sostenibles.
<p/>
En definitiva, conviene que los gobiernos y 
las empresas colaboren para que el consumo responsable
 sea accesible. 
  ...
  answersShuffled.map((question) => {
      ...
      <button onClick={giveUpShowAnswers}>Poddaj Się - Sprawdź odpowiedzi</button>
        ...
        {result === false && <h1>Nie wszystko dobrze </h1>}
         ...
         <button onClick={() => navigate("/TextCompletion5")}
          disabled={!(taskFinished && checkAnsers() === correctAnsers.length)}
          >Zadanie 3</button>
      </div>
...
export default TextCompletion4;